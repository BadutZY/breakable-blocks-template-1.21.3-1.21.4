...
public class ObtainableEnd implements ModInitializer {
	...
	public void onInitialize() {
		...
		// Intercept block attack attempts untuk spawner
		AttackBlockCallback.EVENT.register((player, world, hand, pos, direction) -> {
			if (world.getBlockState(pos).isOf(Blocks.SPAWNER)) {
				...
				if (!isValidPickaxe(heldItem)) {
					LOGGER.info("Blocking spawner attack with invalid tool: {} ({})",
							heldItem.getItem().toString(),
							heldItem.getItem().getTranslationKey());
					return ActionResult.FAIL;
				}
				LOGGER.info("Allowing spawner attack with valid tool: {} ({})",
						heldItem.getItem().toString(),
						heldItem.getItem().getTranslationKey());
			}
			// Check untuk End Portal Frame
			if (world.getBlockState(pos).isOf(Blocks.END_PORTAL_FRAME)) {
				...
				ItemStack heldItem = player.getMainHandStack();
				if (!isValidPickaxe(heldItem)) {
					LOGGER.info("Blocking end portal frame attack with invalid tool: {} ({})",
							heldItem.getItem().toString(),
							heldItem.getItem().getTranslationKey());
					return ActionResult.FAIL;
				}
				LOGGER.info("Allowing end portal frame attack with valid tool: {} ({})",
						heldItem.getItem().toString(),
						heldItem.getItem().getTranslationKey());
			}
			return ActionResult.PASS;
		...
		PlayerBlockBreakEvents.AFTER.register((world, player, pos, state, blockEntity) -> {
			...
			if (state.isOf(Blocks.SPAWNER) && isValidPickaxe(heldItem)) {
				// Always drop spawner with any pickaxe (no silk touch required)
				...
				if (blockEntity instanceof MobSpawnerBlockEntity spawnerEntity) {
					...
					if (!animalEgg.isEmpty()) {
						...
						LOGGER.info("Spawn egg dropped: {}", animalEgg.getItem().toString());
					}
				}
			}

			// Handle End Portal Frame drops HANYA untuk vanilla pickaxe
			// Custom pickaxe dari mod akan menggunakan loot table
			if (state.isOf(Blocks.END_PORTAL_FRAME) && isValidPickaxe(heldItem)) {
				// Check apakah pickaxe adalah vanilla (ada di ItemTags.PICKAXES)
				boolean isVanillaPickaxe = heldItem.isIn(ItemTags.PICKAXES);

				if (isVanillaPickaxe) {
					// Vanilla pickaxe: drop manual karena loot table kadang tidak trigger
					dropItem(world, pos, new ItemStack(Items.END_PORTAL_FRAME));
					LOGGER.info("End Portal Frame broken and dropped with vanilla pickaxe: {}", heldItem.getItem().toString());

					// Drop ender eye if present
					...
					// Custom pickaxe: biarkan loot table yang handle
					LOGGER.info("End Portal Frame broken with custom pickaxe: {} - using loot table", heldItem.getItem().toString());
				}
			}
		});
	}
	...
	public static boolean isValidPickaxe(ItemStack itemStack) {
		...
		// Method 1: Check ItemTags.PICKAXES (vanilla dan mod yang register dengan benar)
		if (itemStack.isIn(ItemTags.PICKAXES)) {
			return true;
		}

		// Method 2: Check nama item mengandung "pickaxe" (case insensitive)
		String itemName = itemStack.getItem().toString().toLowerCase();
		if (itemName.contains("pickaxe")) {
			return true;
		}

		// Method 3: Check translation key mengandung "pickaxe"
		String translationKey = itemStack.getItem().getTranslationKey().toLowerCase();
		if (translationKey.contains("pickaxe")) {
			return true;
		}

		// Method 4: Check apakah item bisa mining block dengan kecepatan yang sesuai
		// Item yang bisa mine spawner/end portal frame dengan cepat kemungkinan adalah pickaxe
		if (itemStack.isSuitableFor(Blocks.SPAWNER.getDefaultState()) ||
				itemStack.isSuitableFor(Blocks.END_PORTAL_FRAME.getDefaultState())) {
			return true;
		}
